...
- Install [uv](https://docs.astral.sh/uv/getting-started/installation/) on a supported Python version (3.9+).
- Clone this repo and install dependencies by running: `uv sync --group dev`.
- In the `app` directory, use `uv run python main.py` to run the app.
- If you want to build the app locally, run `uv run pyinstaller main.spec` in the `build` directory.
